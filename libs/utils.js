...
module.exports = {
     ...
     * */
    messageFormat(msg, logger) {
        try {
            const msgData = Object.assign({}, msg);
            ...
            if (logger) {
                logger.error(`[messageFormat] Bad msg: ${inspect(msg)}`);
            }
            return '{}';
        }
    },
    ...
    hsb2rgb(hsb) {
        ...
        for (let offset = 240, i = 0; i < 3; i++, offset -= 120) {
            ...
            const x = Math.abs(((hsb[0] + offset) % 360) - 240);
            //如果相差小于60°则为255
            if (x <= 60) rgb[i] = 255;
            //如果相差在60°和120°之间，
